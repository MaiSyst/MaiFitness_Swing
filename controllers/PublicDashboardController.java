...
public class PublicDashboardController {
    ...
    private static final List<MaiState> maiState=new ArrayList<>();
    ...
    private final JButton btnRefreshSubscribe;
    public PublicDashboardController(
        ...
        this.btnRefreshSubscribe=btnRefreshSubscribe;
        this.parent = parent;
        this.roomId = roomId;
        ...
        if (subsDefault != null) {
            priceSubscription.setText(MaiUtils.numberFormat(subsDefault.price()) + " FCFA");
        }
        ...
        btnRefreshSubscribe.addActionListener(l->{
            maiState.forEach(state->state.updateState());
            fetchActivities();
            fetchSubscription();
...
//            fetchSubscription();
                ...
                fetch.get(Constants.CUSTOMER_CHECK_URL_PATH + "/" + roomId + "/" + identity).then((result, status) -> {
                    if (status == ResponseStatusCode.OK) {
                        ...
                        new PopupValidateCustomerController(parent, 
                                () -> maiState.forEach(state -> state.updateState(
                                customer.identityEMF(),
                                customer.firstName(),
                                customer.lastName(),
                                customer.address(),
                                customer.yearOfBirth(),
                                customer.roomName(),
                                customer.subscribes().get(0).dateEnd()))
                        ).show(
                                identity,
                                customer.firstName(),
                                customer.lastName(),
                                ...
                                body).then((result, status) -> {
                                    if (status == ResponseStatusCode.OK) {
                                        ...
                                        maiState.forEach(state -> state.updateState(
                                                ...
                                                customerModel.roomName(),
                                                customerModel.subscribes().get(0).dateEnd()
                                        ));
    ...
    public void subscribe(MaiState... maiStates) {
        maiState.addAll(Arrays.asList(maiStates));
    }

    private String validateMessage(String dateEnd) {
        if (dateEnd.equals("Expirer")) {
