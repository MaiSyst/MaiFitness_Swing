...
public final class PlanningModalController {
    ...
    public PlanningModalController(final JFrame parent, 
    ...
    public PlanningModalController(final JFrame parent, final MaiFetch fetch, final String planningId,
        ...
        for (var i = 0; i < planningModal.getComboActivities().getItemCount(); i++) {
            if (planningModal.getComboActivities().getItemAt(i).label().toLowerCase()
                    .equals(activity.toLowerCase())) {
                planningModal.getComboActivities().setSelectedIndex(i);
                break;
            }
        }
        for (var i = 0; i < planningModal.getComboRoom().getItemCount(); i++) {
            if (planningModal.getComboRoom().getItemAt(i).roomName().toLowerCase()
    ...
    private void fetchRoom() {
     ...
     private void fetchRoom(String roomId) {
        try {
            fetch.get(Constants.ROOM_FETCH_BY_ID_URL_PATH+"/"+roomId).then((result, status) -> {
                if (status == ResponseStatusCode.OK) {
                ...
                fetch.post(Constants.PLANNING_ADD_URL_PATH + activityId + "/" + roomId, body)
                        .then((result, status) -> {
                            if (status == ResponseStatusCode.OK) {
                                ...
                                if(maiState!=null){maiState.updateState();}
                            } else {
